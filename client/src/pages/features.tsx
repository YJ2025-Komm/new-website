...
export default function Features() {
  return (
    <div className="min-h-screen text-slate-900" style={{ overflowX: 'clip' }}>
      ...
      {/* Circular Diagram Section — How It All Connects */}
      <section className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="hidden md:block relative w-80 h-80 sm:w-96 sm:h-96 mx-auto mb-8">
            {/* SVG dashed circle arc */}
            <svg className="absolute inset-0 w-full h-full" style={{ pointerEvents: 'none' }}>
              <circle cx="50%" cy="50%" r="42%" fill="none"
                      stroke="url(#circleGrad)" strokeWidth="1.5" strokeDasharray="6 4" opacity="0.4"/>
              <defs>
            ...
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-full flex items-center justify-center shadow-2xl" style={{ background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)' }}>
                <p className="text-white font-black text-xs text-center leading-tight px-2">
                  AI Visibility<br/>Intelligence
                </p>
              </div>
            </div>

            {/* Pentagon vertices — 5 nodes */}
            {/* Top center */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2 glass rounded-2xl px-3 py-2 w-32 text-center shadow-sm">
              <p className="text-xs font-black text-slate-800 leading-tight">Query Modeling</p>
            </div>
            {/* Top-right */}
            <div className="absolute top-[22%] right-0 glass rounded-2xl px-3 py-2 w-32 text-center shadow-sm">
              <p className="text-xs font-black text-slate-800 leading-tight">Multi Model Capture</p>
            </div>
            {/* Bottom-right */}
            <div className="absolute bottom-[8%] right-[4%] glass rounded-2xl px-3 py-2 w-32 text-center shadow-sm">
              <p className="text-xs font-black text-slate-800 leading-tight">Signal Interpretation</p>
            </div>
            {/* Bottom-left */}
            <div className="absolute bottom-[8%] left-[4%] glass rounded-2xl px-3 py-2 w-32 text-center shadow-sm">
              <p className="text-xs font-black text-slate-800 leading-tight">Prescriptive Action</p>
            </div>
            {/* Top-left */}
            <div className="absolute top-[22%] left-0 glass rounded-2xl px-3 py-2 w-32 text-center shadow-sm">
              <p className="text-xs font-black text-slate-800 leading-tight">Visibility Improvement</p>
            </div>
          </div>

          {/* Mobile: vertical numbered list */}
          <div className="md:hidden space-y-2 max-w-xs mx-auto mb-8">
            {['Query Modeling', 'Multi Model Capture', 'Signal Interpretation', 'Prescriptive Action', 'Visibility Improvement'].map((label, i) => (
              <div key={i} className="flex items-center gap-3 glass rounded-2xl p-3">
                <div
                ...
                {i < 4 && <span className="ml-auto text-slate-300 text-sm">→</span>}
              </div>
            ))}
          </div>
        </div>
